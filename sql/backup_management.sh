...
#============================================================================
# This is a test script for backup management of pg_arman.
#============================================================================
...
pg_arman backup -B ${BACKUP_PATH} -b full -Z -p ${TEST_PGPORT} -d postgres --quiet;echo $?
pg_arman validate -B ${BACKUP_PATH} --quiet
pg_arman show detail -B ${BACKUP_PATH} > ${TEST_BASE}/TEST-0001-before.log 2>&1
grep OK ${TEST_BASE}/TEST-0001-before.log | sed -e 's@[^-]@@g' | wc -c
pg_arman backup -B ${BACKUP_PATH} -b full --keep-data-generations=1 -Z -p ${TEST_PGPORT} -d postgres --quiet;echo $?
#FOR DEBUG
#pg_arman backup -B ${BACKUP_PATH} -b full --keep-data-generations=1 -Z -p ${TEST_PGPORT} -d postgres;echo $?
pg_arman validate -B ${BACKUP_PATH} --quiet
pg_arman show detail --show-all -B ${BACKUP_PATH} > ${TEST_BASE}/TEST-0001-after.log 2>&1
...
echo '###### keep generations for full and page backup ######'
pg_arman backup -B ${BACKUP_PATH} -b full -Z -p ${TEST_PGPORT} -d postgres --quiet;echo $?
pg_arman validate -B ${BACKUP_PATH} --quiet
pg_arman backup -B ${BACKUP_PATH} -b full -Z -p ${TEST_PGPORT} -d postgres --quiet;echo $?
pg_arman validate -B ${BACKUP_PATH} --quiet
...
pg_arman validate -B ${BACKUP_PATH} --quiet
pg_arman show detail -B ${BACKUP_PATH} > ${TEST_BASE}/TEST-0002-before.log 2>&1
grep OK ${TEST_BASE}/TEST-0002-before.log | sed -e 's@[^-]@@g' | wc -c
pg_arman backup -B ${BACKUP_PATH} -b full --keep-data-generations=1 -Z -p ${TEST_PGPORT} -d postgres > /dev/null 2>&1;echo $?
# FOR DEBUG
#pg_arman backup -B ${BACKUP_PATH} -b full --keep-data-generations=1 -Z -p ${TEST_PGPORT} -d postgres;echo $?
pg_arman validate -B ${BACKUP_PATH} --quiet
pg_arman show detail --show-all -B ${BACKUP_PATH} > ${TEST_BASE}/TEST-0002-after.log 2>&1
...
pg_arman backup -B ${BACKUP_PATH} -b full -Z -p ${TEST_PGPORT} -d postgres --quiet;echo $?
pg_arman validate -B ${BACKUP_PATH} --quiet
pg_arman backup -B ${BACKUP_PATH} -b arc -Z -p ${TEST_PGPORT} -d postgres --quiet;echo $?
pg_arman validate -B ${BACKUP_PATH} --quiet
pg_arman show detail -B ${BACKUP_PATH} > ${TEST_BASE}/TEST-0003-before.log 2>&1
grep OK ${TEST_BASE}/TEST-0003-before.log | sed -e 's@[^-]@@g' | wc -c
pg_arman backup -B ${BACKUP_PATH} -b full --keep-data-generations=1 -Z -p ${TEST_PGPORT} -d postgres --quiet;echo $?
# FOR DEBUG
#pg_arman backup -B ${BACKUP_PATH} -b full --keep-data-generations=1 -Z -p ${TEST_PGPORT} -d postgres;echo $?
pg_arman validate -B ${BACKUP_PATH} --quiet
pg_arman show detail --show-all -B ${BACKUP_PATH} > ${TEST_BASE}/TEST-0003-after.log 2>&1
...
pg_arman show detail -B ${BACKUP_PATH} > ${TEST_BASE}/TEST-0004-before.log 2>&1
grep OK ${TEST_BASE}/TEST-0004-before.log | sed -e 's@[^-]@@g' | wc -c
pg_arman backup -B ${BACKUP_PATH} -b full --keep-data-generations=1 -Z -p ${TEST_PGPORT} -d postgres > /dev/null 2>&1;echo $?
# FOR DEBUG
#pg_arman backup -B ${BACKUP_PATH} -b full --keep-data-generations=1 -Z -p ${TEST_PGPORT} -d postgres;echo $?
pg_arman validate -B ${BACKUP_PATH} --quiet
pg_arman show detail --show-all -B ${BACKUP_PATH} > ${TEST_BASE}/TEST-0004-after.log 2>&1
...
pg_arman show detail -B ${BACKUP_PATH} > ${TEST_BASE}/TEST-0005-before.log 2>&1
grep OK ${TEST_BASE}/TEST-0005-before.log | sed -e 's@[^-]@@g' | wc -c
pg_arman backup -B ${BACKUP_PATH} -b full --keep-data-days=1 -Z -p ${TEST_PGPORT} -d postgres --quiet;echo $?
# FOR DEBUG
#pg_arman backup -B ${BACKUP_PATH} -b full --keep-data-days=1 -Z -p ${TEST_PGPORT} -d postgres;echo $?
pg_arman validate -B ${BACKUP_PATH} --quiet
pg_arman show detail --show-all -B ${BACKUP_PATH} > ${TEST_BASE}/TEST-0005-after.log 2>&1
...
echo '###### keep days for full and page backup ######'
pg_arman backup -B ${BACKUP_PATH} -b full -Z -p ${TEST_PGPORT} -d postgres --quiet;echo $?
pg_arman validate -B ${BACKUP_PATH} --quiet
create_dummy_backup 1 PAGE OK
create_dummy_backup 2 FULL OK
create_dummy_backup 3 FULL OK
pg_arman show detail -B ${BACKUP_PATH} > ${TEST_BASE}/TEST-0006-before.log 2>&1
grep OK ${TEST_BASE}/TEST-0006-before.log | sed -e 's@[^-]@@g' | wc -c
pg_arman backup -B ${BACKUP_PATH} -b full --keep-data-days=1 -Z -p ${TEST_PGPORT} -d postgres > /dev/null 2>&1;echo $?
# FOR DEBUG
#pg_arman backup -B ${BACKUP_PATH} -b full --keep-data-days=1 -Z -p ${TEST_PGPORT} -d postgres;echo $?
pg_arman validate -B ${BACKUP_PATH} --quiet
pg_arman show detail --show-all -B ${BACKUP_PATH} > ${TEST_BASE}/TEST-0006-after.log 2>&1
...
create_dummy_backup 2 FULL OK
create_dummy_backup 3 FULL OK
pg_arman show detail -B ${BACKUP_PATH} > ${TEST_BASE}/TEST-0007-before.log 2>&1
grep OK ${TEST_BASE}/TEST-0007-before.log | sed -e 's@[^-]@@g' | wc -c
pg_arman backup -B ${BACKUP_PATH} -b full --keep-data-days=1 -Z -p ${TEST_PGPORT} -d postgres > /dev/null 2>&1;echo $?
# FOR DEBUG
#pg_arman backup -B ${BACKUP_PATH} -b full --keep-data-days=1 -Z -p ${TEST_PGPORT} -d postgres;echo $?
pg_arman validate -B ${BACKUP_PATH} --quiet
pg_arman show detail --show-all -B ${BACKUP_PATH} > ${TEST_BASE}/TEST-0007-after.log 2>&1
...
create_dummy_backup 2 FULL OK
pg_arman show detail -B ${BACKUP_PATH} > ${TEST_BASE}/TEST-0008-before.log 2>&1
grep OK ${TEST_BASE}/TEST-0008-before.log | sed -e 's@[^-]@@g' | wc -c
pg_arman backup -B ${BACKUP_PATH} -b full --keep-data-days=1 -Z -p ${TEST_PGPORT} -d postgres > /dev/null 2>&1;echo $?
# FOR DEBUG
#pg_arman backup -B ${BACKUP_PATH} -b full --keep-data-days=1 -Z -p ${TEST_PGPORT} -d postgres;echo $?
pg_arman validate -B ${BACKUP_PATH} --quiet
pg_arman show detail --show-all -B ${BACKUP_PATH} > ${TEST_BASE}/TEST-0008-after.log 2>&1
...
create_dummy_backup 2 FULL OK
create_dummy_backup 3 FULL OK
pg_arman show detail -B ${BACKUP_PATH} > ${TEST_BASE}/TEST-0009-before.log 2>&1
grep OK ${TEST_BASE}/TEST-0009-before.log | sed -e 's@[^-]@@g' | wc -c
echo "keep data generation : 1, keep data days : 2"
pg_arman backup -B ${BACKUP_PATH} -b full --keep-data-generations=1 --keep-data-days=2 -Z -p ${TEST_PGPORT} -d postgres > /dev/null 2>&1;echo $?
# FOR DEBUG
#pg_arman backup -B ${BACKUP_PATH} -b full --keep-data-generations=1 --keep-data-days=2 -Z -p ${TEST_PGPORT} -d postgres;echo $?
pg_arman validate -B ${BACKUP_PATH} --quiet
pg_arman show detail --show-all -B ${BACKUP_PATH} > ${TEST_BASE}/TEST-0009-after.log 2>&1
...
create_dummy_backup 2 FULL OK
create_dummy_backup 3 FULL OK
pg_arman show detail -B ${BACKUP_PATH} > ${TEST_BASE}/TEST-0010-before.log 2>&1
grep OK ${TEST_BASE}/TEST-0010-before.log | sed -e 's@[^-]@@g' | wc -c
echo "keep data generation : 3, keep data days : 1"
pg_arman backup -B ${BACKUP_PATH} -b full --keep-data-generations=3 --keep-data-days=1 -Z -p ${TEST_PGPORT} -d postgres > /dev/null 2>&1;echo $?
# FOR DEBUG
#pg_arman backup -B ${BACKUP_PATH} -b full --keep-data-generations=3 --keep-data-days=1 -Z -p ${TEST_PGPORT} -d postgres;echo $?
pg_arman validate -B ${BACKUP_PATH} --quiet
pg_arman show detail --show-all -B ${BACKUP_PATH} > ${TEST_BASE}/TEST-0010-after.log 2>&1
...
echo '###### keep generations and days together with full and page backup ######'
pg_arman backup -B ${BACKUP_PATH} -b full -Z -p ${TEST_PGPORT} -d postgres --quiet;echo $?
pg_arman validate -B ${BACKUP_PATH} --quiet
create_dummy_backup 1 FULL OK
create_dummy_backup 2 PAGE OK
create_dummy_backup 3 FULL OK
create_dummy_backup 4 FULL OK
pg_arman show detail -B ${BACKUP_PATH} > ${TEST_BASE}/TEST-0011-before.log 2>&1
grep OK ${TEST_BASE}/TEST-0011-before.log | sed -e 's@[^-]@@g' | wc -c
echo "keep data generation : 2, keep data days : 2"
pg_arman backup -B ${BACKUP_PATH} -b full --keep-data-generations=2 --keep-data-days=2 -Z -p ${TEST_PGPORT} -d postgres > /dev/null 2>&1;echo $?
# FOR DEBUG
#pg_arman backup -B ${BACKUP_PATH} -b full --keep-data-generations=2 --keep-data-days=2 -Z -p ${TEST_PGPORT} -d postgres;echo $?
pg_arman validate -B ${BACKUP_PATH} --quiet
pg_arman show detail --show-all -B ${BACKUP_PATH} > ${TEST_BASE}/TEST-0011-after.log 2>&1
...
create_dummy_backup 3 FULL OK
create_dummy_backup 4 FULL OK
pg_arman show detail -B ${BACKUP_PATH} > ${TEST_BASE}/TEST-0012-before.log 2>&1
...
#pg_arman backup -B ${BACKUP_PATH} -b full --keep-data-generations=2 --keep-data-days=2 -Z -p ${TEST_PGPORT} -d postgres;echo $?
pg_arman validate -B ${BACKUP_PATH} --quiet
pg_arman show detail --show-all -B ${BACKUP_PATH} > ${TEST_BASE}/TEST-0012-after.log 2>&1
...
create_dummy_backup 3 FULL OK
create_dummy_backup 4 FULL OK
pg_arman show detail -B ${BACKUP_PATH} > ${TEST_BASE}/TEST-0013-before.log 2>&1
...
#pg_arman backup -B ${BACKUP_PATH} -b full --keep-data-generations=2 --keep-data-days=2 -Z -p ${TEST_PGPORT} -d postgres;echo $?
pg_arman validate -B ${BACKUP_PATH} --quiet
pg_arman show detail --show-all -B ${BACKUP_PATH} > ${TEST_BASE}/TEST-0013-after.log 2>&1
