...
#============================================================================
# This is a test script for archived WAL and serverlog files management of pg_arman.
#============================================================================
...
#ls -l ${ARCLOG_PATH}
pg_arman backup -B ${BACKUP_PATH} -b page -Z --keep-arclog-files=3 -p ${TEST_PGPORT} -d postgres > /dev/null 2>&1; echo $?
# FOR DEBUG
#pg_arman backup -B ${BACKUP_PATH} -b page -Z --keep-arclog-files=3 -p ${TEST_PGPORT} -d postgres --debug; echo $?
...
pg_arman backup -B ${BACKUP_PATH} -b page -Z --keep-arclog-days=1 -p ${TEST_PGPORT} -d postgres > /dev/null 2>&1; echo $?
# FOR DEBUG
#pg_arman backup -B ${BACKUP_PATH} -b page -Z --keep-arclog-days=1 -p ${TEST_PGPORT} -d postgres --debug; echo $?
NUM_OF_ARCLOG_FILES_AFTER=`ls ${ARCLOG_PATH} | grep -v backup | wc -l`
echo "Number of remaining archivelog files: ${NUM_OF_ARCLOG_FILES_AFTER}"
#ls -l ${ARCLOG_PATH}
...
#ls -l ${ARCLOG_PATH}
pg_arman backup -B ${BACKUP_PATH} -b page -Z --keep-arclog-files=3 --keep-arclog-days=1 -p ${TEST_PGPORT} -d postgres > /dev/null 2>&1; echo $?
# FOR DEBUG
#pg_arman backup -B ${BACKUP_PATH} -b page -Z --keep-arclog-files=3 --keep-arclog-days=1 -p ${TEST_PGPORT} -d postgres --debug; echo $?
NUM_OF_ARCLOG_FILES_AFTER=`ls ${ARCLOG_PATH} | grep -v backup | wc -l`
echo "Number of remaining archivelog files: ${NUM_OF_ARCLOG_FILES_AFTER}"
#ls -l ${ARCLOG_PATH}
...
pg_arman backup -B ${BACKUP_PATH} -b full -Z -s -p ${TEST_PGPORT} -d postgres --quiet;echo $?
pg_arman validate -B ${BACKUP_PATH} --quiet
...
#ls -l ${SRVLOG_PATH}
pg_arman backup -B ${BACKUP_PATH} -b arc -Z -s --keep-srvlog-files=3 -p ${TEST_PGPORT} -d postgres > /dev/null 2>&1;echo $?
# FOR DEBUG
#pg_arman backup -B ${BACKUP_PATH} -b arc -Z -s --keep-srvlog-files=3 -p ${TEST_PGPORT} -d postgres --debug;echo $?
...
#ls -l ${SRVLOG_PATH}
pg_arman backup -B ${BACKUP_PATH} -b arc -Z -s --keep-srvlog-days=1 -p ${TEST_PGPORT} -d postgres > /dev/null 2>&1;echo $?
# FOR DEBUG
#pg_arman backup -B ${BACKUP_PATH} -b arc -Z -s --keep-srvlog-days=1 -p ${TEST_PGPORT} -d postgres --debug;echo $?
NUM_OF_SRVLOG_FILES_AFTER=`ls ${SRVLOG_PATH} | grep -v backup | wc -l`
echo "Number of remaining server log files: ${NUM_OF_SRVLOG_FILES_AFTER}"
#ls -l ${SRVLOG_PATH}
...
#ls -l ${SRVLOG_PATH}
pg_arman backup -B ${BACKUP_PATH} -b arc -Z -s --keep-srvlog-files=4 --keep-srvlog-days=1 -p ${TEST_PGPORT} -d postgres > /dev/null 2>&1;echo $?
# FOR DEBUG
#pg_arman backup -B ${BACKUP_PATH} -b arc -Z -s --keep-srvlog-files=4 --keep-srvlog-days=1 -p ${TEST_PGPORT} -d postgres --debug;echo $?
NUM_OF_SRVLOG_FILES_AFTER=`ls ${SRVLOG_PATH} | grep -v backup | wc -l`
echo "Number of remaining server log files: ${NUM_OF_SRVLOG_FILES_AFTER}"
#ls -l ${SRVLOG_PATH}
